    ...
    render(){
        return(
            <div className='status-effect-form column'>
                ...
                <button className='closeButton' onClick={()=>this.props.closeStatusForm()}> x</button>
                <div className=''>
                    <div>character id = <i>{this.props.characterid}</i></div>
                    <div>Status Effect Name: <input className="inputboxMedium roundedbox" 
                        type="text"
                        onChange={(e) => this.handleInputChange(e, "inputName")}
                    /></div>
                    <div><b>Effected Stats</b></div>
                    {["HP", "STR", "DEX", "CON", "INT", "WIS", "CHA"].map((stat)=>(
                        <span>{stat}: <input className="inputboxSmall roundedbox inline" 
                        type="number"
                        onChange={(e) => this.handleStatChange(e, stat)}
                    /></span>
                    ))}
                    <div>Effect Duration: <input className="inputboxSmall roundedbox" 
                        type="number" min="1"
                        onChange={(e) => this.handleInputChange(e, "inputDuration")}
                    /></div>
                    <div>Description: <textarea rows={2} id={"growable"} onChange={(e) => {
                        this.handleInputChange(e, "inputDescription")
                    }} value={this.state.inputDescription}/></div>
                    <button className='submitStatusButton' onClick={()=>this.submitStatus()}>Submit</button>
                </div>
            </div>
        )
    }
